    ...
    for ann in anns:
        ...
        if major_axis_length is not None and minor_axis_length is not None:
            result = {
                ...
                'file_name': 'nisan' + extract_number_after_mayis(image_info['file_name'])
            }
...
depth_results = []
for image_info in depth_coco_data['images']:
    image_id = image_info['id']
    file_name = image_info['file_name']
    ...
    for ann in anns:
        ...
        if major_axis_length is not None and minor_axis_length is not None:
            ...
            points = np.array(segmentation).reshape((-1, 2)).astype(np.int32)
            cv2.polylines(depth_visual_colored, [points], isClosed=True, color=(0, 0, 0), thickness=1)
